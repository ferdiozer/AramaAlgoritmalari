...
namespace AramaAlgoritma.ErdincHoca
{
    /// <summary>
    /// Erdinc Hocanın github hesabından alınmıştır. 
    /// https://github.com/erdincuzun/SMA.NET
    /// </summary>
    class MaximalShift : AlgoritmaBase
    {
        public Pattern[] pat = null;
        public int[] adaptedGs = null;
        public int[] qsBc = null;
        public int[] minShift = null;
        public char[] x = null;
        public MaximalShift(string Metin, string AramaMetin) : base(Metin, AramaMetin)
        {
        ...
        public bool PrefixOlustur() {
            x = AramaMetin.ToCharArray();
            pat = new Pattern[AramaMetin.Length];
            adaptedGs = new int[AramaMetin.Length + 1]; qsBc = new int[65536];
            minShift = ComputeMinShift(x);
            PreQsBc(x, ref qsBc);
            OrderPattern(x, ref pat, ref minShift);
            PreAdaptedGs(x, ref adaptedGs, pat);
        ...
        public List<int> Search(string pattern, string source)
        {
            char[] y = source.ToCharArray();
            int i, j, m = AramaMetin.Length, n = y.Length;
            ...
            {
                i = 0;
                while (i < m && ++Karsilastirma >= 0 && pat[i].c == y[j + pat[i].loc])
                    ++i;
                if (i >= m) { 
                    result.Add(j);
                    EslesenIndexEkle(j);
